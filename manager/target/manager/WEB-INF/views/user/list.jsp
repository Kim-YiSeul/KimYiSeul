	...
	<div id="school_list" style="width:90%; margin:auto; margin-top:5rem;">
		...
		<div class="school_list_excel">
			<table class="table table-hover" style="border:1px solid #c4c2c2;">
				...
				<tr>
					<td>${user.rNum}</td>
				    <th scope="row" class="table-thead-sub" style="border: 1px solid #c4c2c2;"><a href="/user/update?userUID=${user.userUID}" name="userUpdate">${user.userId}</a></th>
				    <td>${user.userName}</td>
				    <td>${user.userNick}</td>
				...
				<c:if test="${empty list}">
				<tr>
				    <td colspan="6">등록된 회원정보가 없습니다.</td>
				</tr>	
				</c:if>
				</tbody>
			</table>
